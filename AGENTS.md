...
This Next.js 16 App Router project keeps routed pages inside `app/`, with marketing subroutes (e.g., `app/pressekit`) beside shared layout artifacts. UI building blocks live under `components/sections` (composed blocks) and `components/ui` (primitives), while lightweight helpers stay in `lib/`. Static assets and favicons sit in `public/`, and playbooks plus briefs live in `docs/` for reference.
